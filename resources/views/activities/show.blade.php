...
@section('content')
    <div class="max-w-2xl mx-auto px-4 py-8">
        <h1 class="text-xl font-bold text-white tracking-tight uppercase mb-6">Detalhes da Atividade</h1>
        ...
        <div class="bg-brand-dark-card border border-brand-dark-border rounded-2xl p-8">
            ...
            <h2 class="text-2xl font-extrabold text-white mt-4">{{ $activity->title }}</h2>
            ...
            <div class="grid grid-cols-2 gap-4 mt-6">
                <div class="bg-brand-dark-input rounded-lg p-4 border border-brand-dark-border">
                    ...
                    <p class="text-white font-semibold">{{ $activity->date_time->format('d/m/Y \à\s H\hi') }}</p>
                </div>
                <div class="bg-brand-dark-input rounded-lg p-4 border border-brand-dark-border">
                    ...
                    <p class="text-white font-semibold {{ $activity->isExpired() ? 'text-red-400' : '' }}">
                        {{ $activity->deadline->format('d/m/Y \à\s H\hi') }}
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                ...
                <button id="btn-share-native" onclick="shareNative()" class="hidden items-center gap-2 bg-brand-yellow hover:bg-brand-yellow-hover text-brand-dark font-bold py-2.5 px-5 rounded-lg transition text-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>
                    Compartilhar
                </button>

                {{-- Botão WhatsApp (fallback desktop) --}}
                <a id="btn-share-wa" href="https://wa.me/?text={{ urlencode($activity->title . "\n\nTipo: " . $activity->typeLabel() . "\nData: " . $activity->date_time->format('d/m/Y') . " as " . $activity->date_time->format('H:i') . "\nPrazo: " . $activity->deadline->format('d/m/Y') . " as " . $activity->deadline->format('H:i') . ($activity->location ? "\nLocal: " . $activity->location : "") . "\nPontos: +" . $activity->points . ($activity->description ? "\n\n" . $activity->description : "") . "\n\n" . route('activities.show', $activity)) }}" target="_blank" rel="noopener noreferrer"
                    class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-5 rounded-lg transition text-sm">
            ...
            <div class="mt-8">
                ...
                @forelse ($activity->confirmedParticipants as $participant)
                    <div class="flex items-center gap-3 mb-3 last:mb-0 p-3 bg-brand-dark-input rounded-lg border border-brand-dark-border">
                        ...
                        <div>
                            <p class="text-sm text-white font-medium">{{ $participant->name }}</p>
